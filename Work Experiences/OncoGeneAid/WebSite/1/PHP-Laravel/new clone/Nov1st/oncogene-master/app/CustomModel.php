...
class CustomModel extends  Model
{
    ...
    public  static  function CPaginate($request,$filters=[],$relations=[]){
        ...
        $string='?';
        $result=self::orderBy('id','DESC')->with($relations);
        ...
        if(sizeof($filters)>0){
            foreach ($filters as $key=>$array){
                if(sizeof($array)===3){
                    if(!empty($array[2])){
                        if($array[1]==='like'){
                            $result=$result->where($array[0],'like','%'.$array[2].'%');
                    ...
                    if(!empty($array[1])){
                        if(is_array($array[1])){
                            $result=$result->whereIn($array[0],$array[1]);
                        }
                        else{
                            $result=$result->where($array[0],$array[1]);
                        }
