<?php $desktop_main_cat63=array (
  'rows' =>
  array (
    0 =>
     array(
       'id' => 'cat_slides',
       'child' =>
      array (
        0 =>
         array(
           'type' => 'html',
           'tag' => 'slide',
           'id' => 'cat_slides-slide1',
        ),
      ),
    ),
    1 =>
     array(
       'id' => 'products1',
       'child' =>
      array (
        0 =>
         array(
           'type' => 'widget',
           'tag' => 'horizontal_product_list',
           'id' => 'products1-horizontal_product_list4',
        ),
      ),
    ),
    2 =>
     array(
       'id' => 'ads1',
       'child' =>
      array (
        0 =>
         array(
           'type' => 'html',
           'tag' => 'img',
           'id' => 'ads1-img3',
        ),
        1 =>
         array(
           'type' => 'html',
           'tag' => 'img',
           'id' => 'ads1-img4',
        ),
      ),
    ),
    3 =>
     array(
       'id' => 'ads2',
       'child' =>
      array (
        0 =>
         array(
           'type' => 'html',
           'tag' => 'img',
           'id' => 'ads2-img5',
        ),
        1 =>
         array(
           'type' => 'html',
           'tag' => 'img',
           'id' => 'ads2-img6',
        ),
      ),
    ),
    4 =>
     array(
       'id' => 'cat_info',
       'child' =>
      array (
        0 =>
         array(
           'type' => 'html',
           'tag' => 'p',
           'id' => 'cat_info-p1',
        ),
        1 =>
         array(
           'type' => 'html',
           'tag' => 'p',
           'id' => 'cat_info-p2',
        ),
      ),
    ),
  ),
  'style' =>
   array(
     'cat_info-p2' =>
     array(
       'width' => '',
       'height' => '',
       'backgroundColor' => '',
       'color' => '',
       'marginTop' => '',
       'marginRight' => '',
       'marginLeft' => '',
       'marginBottom' => '',
       'paddingTop' => '',
       'paddingRight' => '',
       'paddingLeft' => '',
       'paddingBottom' => '',
       'borderWidth' => '0px',
       'borderColor' => '',
       'borderTopRightRadius' => '',
       'borderBottomLeftRadius' => '',
       'borderBottomRightRadius' => '',
       'borderTopLeftRadius' => '',
       'flexDirection' => 'row',
       'borderStyle' => 'solid',
       'display' => 'flex',
       'textAlign' => 'right',
       'justifyContent' => 'start',
    ),
     'cat_info-p1' =>
     array(
       'width' => '',
       'height' => '',
       'backgroundColor' => '',
       'color' => '',
       'marginTop' => '',
       'marginRight' => '',
       'marginLeft' => '',
       'marginBottom' => '',
       'paddingTop' => '',
       'paddingRight' => '',
       'paddingLeft' => '',
       'paddingBottom' => '',
       'borderWidth' => '0px',
       'borderColor' => '#FF0000FF',
       'borderTopRightRadius' => '',
       'borderBottomLeftRadius' => '',
       'borderBottomRightRadius' => '',
       'borderTopLeftRadius' => '',
       'flexDirection' => 'row',
       'borderStyle' => 'solid',
       'display' => 'flex',
       'textAlign' => 'right',
       'justifyContent' => 'start',
    ),
     'cat_info' =>
     array(
       'width' => '98%',
       'height' => 'auto',
       'backgroundColor' => '#FFFFFFFF',
       'color' => '',
       'marginTop' => '20px',
       'marginRight' => 'auto',
       'marginLeft' => 'auto',
       'marginBottom' => '',
       'paddingTop' => '20px',
       'paddingRight' => '20px',
       'paddingLeft' => '20px',
       'paddingBottom' => '20px',
       'borderWidth' => '1px',
       'borderColor' => '#F5F5F5FF',
       'borderTopRightRadius' => '',
       'borderBottomLeftRadius' => '',
       'borderBottomRightRadius' => '',
       'borderTopLeftRadius' => '',
       'flexDirection' => 'column',
       'borderStyle' => 'solid',
       'display' => 'flex',
       'textAlign' => 'right',
       'justifyContent' => 'start',
    ),
     'ads2-img6' =>
     array(
       'width' => '98%',
       'height' => '',
       'backgroundColor' => '',
       'color' => '',
       'marginTop' => '',
       'marginRight' => '2%',
       'marginLeft' => '',
       'marginBottom' => '',
       'paddingTop' => '',
       'paddingRight' => '',
       'paddingLeft' => '',
       'paddingBottom' => '',
       'borderWidth' => '0px',
       'borderColor' => '#00000000',
       'borderTopRightRadius' => '10px',
       'borderBottomLeftRadius' => '10px',
       'borderBottomRightRadius' => '10px',
       'borderTopLeftRadius' => '10px',
       'flexDirection' => 'row',
       'borderStyle' => 'solid',
       'display' => 'flex',
       'textAlign' => 'right',
       'justifyContent' => 'start',
    ),
     'ads2-img5' =>
     array(
       'width' => '98%',
       'height' => '',
       'backgroundColor' => '',
       'color' => '',
       'marginTop' => '',
       'marginRight' => '',
       'marginLeft' => '',
       'marginBottom' => '',
       'paddingTop' => '',
       'paddingRight' => '',
       'paddingLeft' => '',
       'paddingBottom' => '',
       'borderWidth' => '0px',
       'borderColor' => '#00000000',
       'borderTopRightRadius' => '10px',
       'borderBottomLeftRadius' => '10px',
       'borderBottomRightRadius' => '10px',
       'borderTopLeftRadius' => '10px',
       'flexDirection' => 'row',
       'borderStyle' => 'solid',
       'display' => 'flex',
       'textAlign' => 'right',
       'justifyContent' => 'start',
    ),
     'ads1-img4' =>
     array(
       'width' => '98%',
       'height' => '',
       'backgroundColor' => '',
       'color' => '',
       'marginTop' => '',
       'marginRight' => '2%',
       'marginLeft' => '',
       'marginBottom' => '',
       'paddingTop' => '',
       'paddingRight' => '',
       'paddingLeft' => '',
       'paddingBottom' => '',
       'borderWidth' => '0px',
       'borderColor' => '#00000000',
       'borderTopRightRadius' => '10px',
       'borderBottomLeftRadius' => '10px',
       'borderBottomRightRadius' => '10px',
       'borderTopLeftRadius' => '10px',
       'flexDirection' => 'row',
       'borderStyle' => 'solid',
       'display' => 'flex',
       'textAlign' => 'right',
       'justifyContent' => 'start',
    ),
     'ads1-img3' =>
     array(
       'width' => '98%',
       'height' => '',
       'backgroundColor' => '',
       'color' => '',
       'marginTop' => '',
       'marginRight' => '',
       'marginLeft' => '',
       'marginBottom' => '',
       'paddingTop' => '',
       'paddingRight' => '',
       'paddingLeft' => '',
       'paddingBottom' => '',
       'borderWidth' => '0px',
       'borderColor' => '#00000000',
       'borderTopRightRadius' => '10px',
       'borderBottomLeftRadius' => '10px',
       'borderBottomRightRadius' => '10px',
       'borderTopLeftRadius' => '10px',
       'flexDirection' => 'row',
       'borderStyle' => 'solid',
       'display' => 'flex',
       'textAlign' => 'right',
       'justifyContent' => 'start',
    ),
     'ads2' =>
     array(
       'width' => '98%',
       'height' => 'auto',
       'backgroundColor' => '',
       'color' => '',
       'marginTop' => '20px',
       'marginRight' => 'auto',
       'marginLeft' => 'auto',
       'marginBottom' => '',
       'paddingTop' => '',
       'paddingRight' => '',
       'paddingLeft' => '',
       'paddingBottom' => '',
       'borderWidth' => '0px',
       'borderColor' => '#00000000',
       'borderTopRightRadius' => '',
       'borderBottomLeftRadius' => '',
       'borderBottomRightRadius' => '',
       'borderTopLeftRadius' => '',
       'flexDirection' => 'row',
       'borderStyle' => 'solid',
       'display' => 'flex',
       'textAlign' => 'right',
       'justifyContent' => 'start',
    ),
     'ads1' =>
     array(
       'width' => '98%',
       'height' => 'auto',
       'backgroundColor' => 'transparent',
       'color' => '',
       'marginTop' => '20px',
       'marginRight' => 'auto',
       'marginLeft' => 'auto',
       'marginBottom' => '',
       'paddingTop' => '',
       'paddingRight' => '',
       'paddingLeft' => '',
       'paddingBottom' => '',
       'borderWidth' => '0px',
       'borderColor' => '#00000000',
       'borderTopRightRadius' => '',
       'borderBottomLeftRadius' => '',
       'borderBottomRightRadius' => '',
       'borderTopLeftRadius' => '',
       'flexDirection' => 'row',
       'borderStyle' => 'solid',
       'display' => 'flex',
       'textAlign' => 'right',
       'justifyContent' => 'space-between',
    ),
     'products1-horizontal_product_list4' =>
     array(
    ),
     'products1' =>
     array(
       'width' => '98%',
       'height' => 'auto',
       'backgroundColor' => '#FFFFFFFF',
       'color' => '',
       'marginTop' => '20px',
       'marginRight' => 'auto',
       'marginLeft' => 'auto',
       'marginBottom' => '',
       'paddingTop' => '',
       'paddingRight' => '',
       'paddingLeft' => '',
       'paddingBottom' => '',
       'borderWidth' => '1px',
       'borderColor' => '#F5F5F5FF',
       'borderTopRightRadius' => '5px',
       'borderBottomLeftRadius' => '5px',
       'borderBottomRightRadius' => '5px',
       'borderTopLeftRadius' => '5px',
       'flexDirection' => 'column',
       'borderStyle' => 'solid',
       'display' => 'flex',
       'textAlign' => 'right',
       'justifyContent' => 'start',
    ),
     'cat_slides-slide1' =>
     array(
       'width' => '',
       'height' => '280px',
       'backgroundColor' => '',
       'color' => '',
       'marginTop' => '',
       'marginRight' => '',
       'marginLeft' => '',
       'marginBottom' => '',
       'paddingTop' => '',
       'paddingRight' => '',
       'paddingLeft' => '',
       'paddingBottom' => '',
       'borderWidth' => '0px',
       'borderColor' => '',
       'borderTopRightRadius' => '10px',
       'borderBottomLeftRadius' => '10px',
       'borderBottomRightRadius' => '10px',
       'borderTopLeftRadius' => '10px',
       'flexDirection' => 'row',
       'borderStyle' => 'solid',
       'display' => 'flex',
       'textAlign' => 'right',
       'justifyContent' => 'start',
    ),
     'cat_slides' =>
     array(
       'width' => '98%',
       'height' => 'auto',
       'backgroundColor' => '',
       'color' => '',
       'marginTop' => '',
       'marginRight' => 'auto',
       'marginLeft' => 'auto',
       'marginBottom' => '',
       'paddingTop' => '',
       'paddingRight' => '',
       'paddingLeft' => '',
       'paddingBottom' => '',
       'borderWidth' => '0px',
       'borderColor' => '',
       'borderTopRightRadius' => '',
       'borderBottomLeftRadius' => '',
       'borderBottomRightRadius' => '',
       'borderTopLeftRadius' => '',
       'flexDirection' => 'column',
       'borderStyle' => 'solid',
       'display' => 'flex',
       'textAlign' => 'right',
       'justifyContent' => 'start',
    ),
  ),
  'tags' =>
   array(
     'horizontal_product_list' =>
     array(
       'products1-horizontal_product_list4' =>
       array(
         'title' => 'پرفروش‌ترین لوازم جانبی',
         'sort' => 0,
         'product_type' => 0,
         'load_type' => 0,
         'catList' => '76',
         'view' => 'horizontal_product_list',
      ),
       'cat_new_products-horizontal_product_list3' =>
       array(
         'title' => 'جدید ترین محصولات  :name',
         'sort' => 0,
         'product_type' => 0,
         'load_type' => 0,
         'catList' => ':id',
         'view' => 'horizontal_product_list',
      ),
       'mobile_products-horizontal_product_list2' =>
       array(
         'title' => 'جدید ترین گوشی های موبایل',
         'sort' => 0,
         'product_type' => 0,
         'load_type' => 1,
         'catList' => '87',
         'view' => 'horizontal_product_list',
      ),
       'new_product-horizontal_product_list1' =>
       array(
         'title' => 'جدید ترین محصولات',
         'sort' => 0,
         'product_type' => 0,
         'load_type' => 0,
         'catList' => '',
         'view' => 'horizontal_product_list',
      ),
    ),
     'slider' =>
     array(
       'slider_right_box-slider1' =>
       array(
         'view' => 'sliders::desktop',
      ),
    ),
     'incredibleOffers' =>
     array(
    ),
     'img' =>
     array(
       'ads2-img6' =>
       array(
         'src' => 'http://localhost/oncogene/public/files/images/1628537311.jpg',
         'link' => '',
         'hoverSrc' => '',
      ),
       'ads2-img5' =>
       array(
         'src' => 'http://localhost/oncogene/public/files/images/1628537323.jpg',
         'link' => '',
         'hoverSrc' => '',
      ),
       'ads1-img4' =>
       array(
         'src' => 'http://localhost/oncogene/public/files/images/1628537299.jpg',
         'link' => '',
         'hoverSrc' => '',
      ),
       'ads1-img3' =>
       array(
         'src' => 'http://localhost/oncogene/public/files/images/1628537334.jpg',
         'link' => '',
         'hoverSrc' => '',
      ),
       'slider_left_box-img2' =>
       array(
         'src' => 'http://localhost/oncogene/public/files/images/0c99fa717c04310ca07b6db54a52d0ffac8fdce3_1598113005.jpg',
         'link' => '',
         'hoverSrc' => '',
      ),
       'slider_left_box-img1' =>
       array(
         'src' => 'http://localhost/oncogene/public/files/images/1773cee3eb3bc9ece26d37cc3d0eda2799218bef_1597479643.jpg',
         'link' => '',
         'hoverSrc' => '',
      ),
    ),
     'card' =>
     array(
    ),
     'ul' =>
     array(
    ),
     'code' =>
     array(
    ),
     'p' =>
     array(
       'cat_info-p2' =>
       array(
         'content' => 'خرید انواع کالای دیجیتال از فروشگاه اینترنتی آنکوژن

عمر انقلاب دیجیتال به کمتر از نیم قرن می‌رسد، اما همین برهه‌ی زمانی کوتاه در تاریخ، زندگی نوع بشر را کاملا دگرگون کرده و شتاب قابل توجهی به پیشرفت انسان در زمینه‌های مختلف بخشیده است. شاید اولین قدم‌های دیجیتالی شدن دنیا را بتوان تغییر تراتزیستورهای آنالوگ به دیجیتال دانست که بستر لازم برای کالای دیجیتال نظیر کامپیوتر، تلفن را فراهم کرد. در حال حاضر محصولات دیجیتالی آنقدر توسعه یافته‌اند که می‌توان ردپای آنها را در ساده‌ترین امور روزمره مشاهده کرد. بعضی از این محصولات به ابزارهای جدانشدنی برای انسان تبدیل شده‌اند، تصور زندگی بدون آنها و امکاناتی که در اختیار انسان قرار می‌دهند، وجود ندارد. موبایل‌ها، لپ‌تاپ، ماشین‌های ادارای از این دسته محصولات هستند. کامپیوترهای شخصی، لپ‌تاپ و گوشی تلفن همراه نیز مسیر پر فراز و نشیبی را پشت سر گذاشته‌اند تا به شکل هوشمند امروزی عرضه شود. اولین کامپیوتر قابل حمل جهان در سال 1981 به بازار آمد. این دستگاه صفحه نمایشی 5 اینچی داشت و وزن آن حدود 11 کیلوگرم بود. در حالیکه شرکت‌هایی نظیر اپل یا هواوی لپ‌تاپ‌هایی به وزن یک کیلوگرم ساخته‌اند.

اولین تلفن همراه نیز در سال 1983 در دنیا به فروش رسید که وزنی معادل یک کیلوگرم داشت و امکانات بسیار کمی را در اختیار داشت. قیمت این دستگاه بیش از سه هزار دلار بود. در حالیکه امروزه شاهد تلفن‌های همراه هوشمندی هستیم که ده‌ها وسیله را در دل خود جای داده‌اند و هر یک از آنها عملکرد فوق‌العاده‌ای رو در اختیار انسان قرار می‌دهد. هم‌چنین محدوده‌ی قیمت این دستگاه بسیار گسترده است و همه‌ی افراد می‌توانند آن را تهیه کنند.

دنیای دیجیتال نه تنها زندگی بشر را راحت‌تر از قبل کرده و زمان بیشتری برای آنها خریده است بلکه وسایل و سرگرمی‌های پیشرفته‌تر و مهیج‌تری را هم فراهم کرده است. تاریخچه‌ی ساخت کنسول بازی نیز مشابه سایر دستگاه‌های دیجیتال است. این صنعت از سال 1972 شروع به کار کرد. اولین کنسول‌های بازی تصویری سیاه و سفید داشتند و محیط بازی آنها بسیار ساده بود. این صنعت با تولید کنسول بازی ایکس باکس در ابتدای قرن 21 ام به اوج خود رسید و دنیای سرگرمی و بازی را متحول کرد. این دستگاه بسیار پیشرفته بود و گرافیک عالی و بازی‌های با کیفیتی را در اختیار علاقمندان قرار می‌داد. کنسول‌های بازی تاکنون 8 نسل را پشت سر گذاشته‌اند. شرکت‌هایی نظیر سونی و مایکروسافت مطرح‌ترین برندهای تولید کننده‌ی کنسول بازی هستند و شرکت‌های متعدد دیگری مثل لاجیتک لوازم جانبی آنها را عرضه می‌کند.

کالاهای دیجیتال به موبایل، لپ تاپ یا وسایل سرگرمی محدود نمی‌شوند. مچ‌بند و ساعت‌های هوشمند از جدیدترین محصولات دیجیتالی به شمار می‌آیند که امکانات زیادی را در اختیار انسان قرار می‌دهد. عملکرد ساعت‌های هوشمند برند اپل آنقدر پیشرفته است که می‌توان آن را یک کامپیوتر مچی در نظر گرفت. شما می‌توانید با ای سیم، تماس‌های اضطراری بین‌المللی برقرار کنید، در وب جستجو کنید و جواب تماس و پیام‌های خود را بدهید. علاوه بر این، اغلب مچ‌بندها و ساعت‌های هوشمند ابزارهای لازم برای پایش سلامت را در اختیار دارند و به طور دائم ضربان قلب، کیفیت خواب، تعداد قدم‌های برداشته شده و ... را محاسبه و گزارش می‌کنند. همچنین در نسخه‌های پیشرفته‌تر می‌توان برنامه‌هایی برای بیماران دیابتی و بهداشت و سلامت بانوان مشاهده کرد.

آنکوژن جدیدترین و به روزترین کالاهای دیجیتال عرضه شده در دنیا را برای مشتریان خود گردآوری کرده است. شما می‌توانید خوشنام‌ترین برندهای جهانی را در وبسایت آنکوژن مشاهده و محصول مورد نظر خود از میان هزاران کالای دیجیتال خریداری کنید.
',
         'showMore' => true,
      ),
       'cat_info-p1' =>
       array(
         'content' => 'کالای دیجیتال',
         'showMore' => '',
      ),
    ),
     'slide' =>
     array(
       'cat_slides-slide1' =>
       array(
         'items' =>
        array (
          0 =>
           array(
             'title' => '',
             'link' => '',
             'src' => 'http://localhost/oncogene/public/files/images/1628494941.jpg',
          ),
          1 =>
           array(
             'title' => '',
             'link' => '',
             'src' => 'http://localhost/oncogene/public/files/images/1628494789.jpg',
          ),
        ),
      ),
    ),
  ),
  'responsive_style' =>
   array(
  ),
)?>
