                       ...
                       formatter:function(){
                           if(this.point.has_product=="ok")
                           {
                               return '<div>' +
                                   ...
                                   '<div style="color:#1457c5;font-size:19px;direction: rtl"><span>'+self.replaceNumber(self.number_format(this.point.price))+'</span> <span>تومان</span></div>' +
                                   '<div>کمترین قیمت</div>' +
                                   '</li>' +
                                   '</ul>' +
                                   '</div>';
                               ...
                               return '<div>' +
                                   '<ul class="chart_ul">' +
                                   '<li style="justify-content: space-between;margin-top: 15px">' +
                                   '<div style="color:#1457c5;font-size:19px;direction: rtl"><span>ناموجود</span></div>' +
                                   '<div>کمترین قیمت</div>' +
                                   '</li>' +
                                   '</ul>' +
                                   '</div>';
                ...
                if(this.chartOptions.series.length==0)
                {
                    ...
                    this.axios.get(url).then(response=>{
                        ...
                        response.data.price.forEach(function (item) {
                            ...
                            app.chartOptions['series'].push({'data':item,'name':name,'color':'#1457c5',marker:{symbol:'circle'},zones:zonesRow,zoneAxis:'x'});
                            if(i>0)
                            {
